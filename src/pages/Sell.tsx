...
import Service3 from "@/components/Heloc/Service3";
...
const Sell = () => {
  ...
  // Service3 has 9 form steps (1-9), so total is 10 (1 selection + 9 form)
  // UniversalService has 16 form steps, so total is 17
  const totalSteps = selectedService === "Sell" ? 10 : 17;
  const progress = (currentStep / totalSteps) * 100;

  // Function to update step from child component
  // UniversalService handles the step mapping (+1) internally
  ...
  const handleFormSubmit = async (data: any) => {
    ...
    const success = await submitLead("Sell", enrichedData, tokens, true);
  ...
  useEffect(() => {
    ...
    if (pre) {
      ...
      // Jump to step 2 (the first form step) - UniversalService handles its own steps
      setCurrentStep(2);
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-primary/5 via-background to-secondary/5 py-8">
      ...
      <div className="container mx-auto px-4 max-w-4xl mt-16">
        ...
        {currentStep === 17 && selectedService !== "Sell" ? (
